...
export function createPromptsModule(
  ...
  const buildSystemPrompt = (cwd: string, memoryContext?: string): string => {
    ...
    return [
      ...
      '',
      '【系统保留标签（禁止输出）】',
      '<historical-assistant-record> ... </historical-assistant-record> 是系统保留标签，仅由系统编排历史回放时使用。你（模型）严禁在任何正式输出中输出该标签，也不得用任何形似标签（如 <...-record>、<...> 这类系统内置标签）包裹自己的正文；你的正式输出必须保持纯净，不要模仿历史回放那种被标签包裹的格式、口吻、详略、样式。若需引用或提及时，用文字描述（如“系统历史回放标签”），不要输出标签本身。',
      '',
      '【工具使用规则】',
  ...
  const buildSupervisorSystemPrompt = (message: string): string => {
    ...
    const base = [
      ...
      '',
      '【系统保留标签（禁止输出）】',
      '<historical-assistant-record> ... </historical-assistant-record> 是系统保留标签，仅由系统编排历史回放时使用。你（管家）严禁在任何正式输出中输出该标签，也不得用任何形似标签（如 <...-record>、<...> 这类系统内置标签）包裹自己的正文；你的正式输出必须保持纯净，不要模仿历史回放那种被标签包裹的格式、口吻、详略、样式。若需引用或提及时，用文字描述（如“系统历史回放标签”），不要输出标签本身。',
      '',
      '你的能力：',
