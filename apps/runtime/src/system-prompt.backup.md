...
- 生成时间：2026-09-02 00:56（约）
- 来源文件：`apps/runtime/src/prompts.ts`
- 来源函数：`buildSystemPrompt`（主 agent 系统提示词）、`buildSupervisorSystemPrompt`（管家提示词）
- 说明：以下为两个函数**最终拼接产出的完整文本**快照。`${...}` 为运行时注入的变量（如环境信息）；`<toolGuide>`、`<builtinSkillCatalog>`、`<memoryContext>` 为运行时动态注入的段落（由 `buildToolGuidePrompt` / `ctx.builtinSkillCatalog` / `buildMemoryContext` 产生，非固定文案）。

---

## 一、主 agent 系统提示词（buildSystemPrompt）
...
<toolGuide：运行时由 buildToolGuidePrompt 动态注入>
...
<builtinSkillCatalog：运行时由 ctx.builtinSkillCatalog 动态注入（含【内置能力】标题）>

<memoryContext：运行时由 buildMemoryContext 动态注入>
...
## 二、管家提示词（buildSupervisorSystemPrompt）
...
<memoryContext：运行时由 buildMemoryContext 动态注入（若有长期记忆命中则追加）>
```