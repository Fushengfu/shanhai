 ...
 * 让 agent 自主调用浏览器工具完成一个查资料任务，并验证：
 * 1. agent 自主调用 browser_* 工具（不是 curl/run_command）
...
app.whenReady().then(async () => {
  try {
    ...
    runtime.onToolTrace((trace) => {
      if (trace.kind === 'tool-call') traces.push(trace.name)
    })

    console.log('=== 开始 agent 任务（真实网关 + 真实浏览器）===')
    const result = await runtime.run(
      '请使用内置浏览器工具打开 https://example.com 这个网页，然后告诉我这个网页的标题和正文主要内容。必须使用浏览器工具（browser_navigate / browser_get_content），不要用 run_command 或 curl。',
    )
    console.log('=== RESULT_START ===')
    console.log(result)
    console.log('=== RESULT_END ===')

    const usedBrowser = traces.some((n) => n.startsWith('browser_'))
    console.log('=== TOOL_TRACES ===')
    console.log(traces.join(' -> '))
    console.log('agent 是否调用浏览器工具:', usedBrowser)
