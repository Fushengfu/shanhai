  ...
  return (
    <>
      ...
      <div style={{ padding: 12, borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        ...
        {ctx.loggedIn && (
          <button onClick={() => void ctx.handleLogout()} title={t('chat.sidebar.logout')} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 4, display: 'inline-flex' }}>
            <IconLogout />
          </button>
        )}
      </div>
