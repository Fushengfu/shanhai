...
> 「创意空间」是山海用来浏览、安装、分享、卸载插件的统一入口，分「发现」「我已安装」「我的提交」等面板。

- **下载**：在「发现」面板浏览已发布的应用，点「下载安装」即可把插件下载到本机并安装。
- **安装**：安装后应用会出现在本机（Dock 应用菜单 / 桌面窗口）可直接使用；本地已安装的应用，回到「发现」面板会显示「已安装」标记、不再显示「下载安装」按钮，避免重复安装。
- **分享**：登录后，在「我已安装」面板对某个插件点「分享 / 提交升级版本共享」，即可把插件提交到创意空间供他人使用；未登录会被明确提示「请先登录」，分享提交走网关鉴权。
- **卸载**：在「我已安装」面板对某个应用点「卸载」，二次确认后从本机移除该插件（卸载不可恢复）。

> 设计理念：山海「一切皆插件」——电脑操作、浏览器自动化、终端、语音、记忆等能力都以插件形态存在，创意空间是这套插件能力的发现与分发入口。
