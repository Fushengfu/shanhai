...
- **会员私信**：登录后与好友一对一收发消息（文本、图片、文件附件），支持未读提醒、未读分割线、右键复制与引用到会话、会话级草稿与窗口尺寸记忆
- **消息分享**：AI 回复与自己发出的消息卡片底部可一键分享给指定好友，内容整条一次发出
- **管家接管私信**：可选开启，好友来信由会话管家直接回复；发送前对疑似密钥、令牌类内容做拦截
- **创意空间应用更新**：作者发布新版本后可在「发现」里直接更新，更新前确认权限变化，更新后可恢复上一版本
- **中英界面**：界面文案支持中文与英文切换，跟随系统语言或手动设置
...
**是什么**：到期窗口展示（token 用量状态栏）、上下文压缩（超阈值自动压历史为摘要）、断点续跑、历史回放隔离（历史 assistant 发言按标准消息原样回放，由系统提示词显式声明其不代表本轮结果，防幻觉）、用户消息标签（`&lt;user_query&gt;` 标注用户原话，靠位置区分当前指令与历史提问，防把旧问题当新任务）。
...
**是什么**：会员账号之间的一对一私信（文本 / 图片 / 文件附件），与会话聊天打通——AI 回复、自己发出的消息都能一键分享给指定好友；可选开启「管家接管」，由会话管家代你回复好友来信。

**能解决什么**：
- 把会话里产出的结论直接发给同事或自己的另一个账号，不必复制粘贴。
- 让管家知道该把消息发给谁：管家可查看好友名单（昵称、用户名、未读数、最后一条消息时间），无需你手报编号。
- 发送与分享前对疑似密钥、令牌、口令与编码数据类内容做拦截，避免把凭证误发出去；被拦时会给出可读原因。

**适合谁**：需要跨设备、跨人协作传递消息，或希望管家代管私信的用户。
...
- **更新**：应用作者发布新版本后，「发现」面板的已安装应用卡片会标出「有更新」，点「更新」即可升级到新版本；服务端版本低于本机时标注「服务端版本较旧」并禁止安装，避免误降级。
- **更新前确认**：新版本申请的能力比已装版本多、或该应用在本机目录里存有额外文件时，安装前会列出差异并请你确认；不同意则不安装，旧版本原样保留。
- **恢复上一版本**：每次更新会自动保留一份更新前的版本备份，「我已安装」面板可一键恢复到上一版本（备份只有一份，恢复后即被消耗；未在本机升级过的应用没有可恢复的备份）。
