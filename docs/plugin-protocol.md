...
插件开发**只有一条工程化链路**（源码字符串 / `node:vm` / `new Function` 快速原型路径已废弃移除）：

| 链路 | 载体 | 适用场景 | 工具流 |
|------|------|----------|--------|
| **工程化** | 独立构建产物（`dist/host.cjs` + `dist/client.html`） | 所有插件（复杂界面、第三方依赖、长期使用） | `plugin_scaffold` → `plugin_build` → `plugin_test_load` → `plugin_verify` → `plugin_install` |

- host 半编译产物（`dist/host.cjs`）由 `require` 加载；client 半窗口入口（`dist/client.html`）由 `loadFile` 加载。
- **不再支持** `code` / `client` 源码字符串（`node:vm` 评估 host 半、`new Function` 渲染 client 半已彻底移除）。

一个「插件」= 一个 **动态 package（DynamicPackage）**，最终由一个**目录**承载（见 §4）。

---

## 1. host 半契约

host 半在**进程内**运行，入口统一为**工厂函数 `(ctx) => disposer`**。唯一加载方式：

### 1.1 工程化：`require` 编译产物（`entryHost` 字段）
...
client 半运行在浏览器渲染进程，**唯一形态**：窗口应用（配合 host 半 `openWindow`），走工程化 `loadFile` 独立入口。

### 2.1 窗口应用形态：`loadFile` 加载 `dist/client.html`（`entryHtml` 字段）
...
> ⚠️ **坑 8（frameless 窗口必读）**：插件窗口与山海其它窗口一样是 **frameless（`frame:false`、无系统标题栏）**，因此 **client 半必须自己提供统一标题栏**，否则「窗口拖不动、关不掉、找不到窗口控制」：
...
>    - 关闭 → `window.shanhaiPlugin.closeApp()`。`closeApp` 默认放行、无需声明 permission。
...
### 3.1 工程化流水线（`plugin_scaffold` → `build` → `test_load` → `verify` → `install`）
...
5. **校验可执行载体**：`dist/host.cjs` / `dist/client.html` 至少其一（部署后 dist 产物才参与判定），否则报错「没有可安装的代码」；
...
### 3.3 openWindow / closeWindow 的配对与自动撤销
...
├── icon.svg             # 可选：Dock 图标（支持 .svg / .png，或 assets/icon.svg）
├── assets/              # 可选：附加静态资源
├── src/                 # 可选：源码（供改后重编译）
└── package.json         # 可选：依赖声明
```

### 4.1 `manifest.json` 完整 schema（`InstalledPackageMeta`）
...
interface InstalledPackageMeta {
  ...
  icon?: string                    // 图标相对路径（相对插件目录，支持 .svg / .png，如 icon.svg / icon.png / assets/icon.svg）
  assets?: string[]                // 附加资源相对路径索引
  dependencies?: Record<string, string>  // 依赖声明（包名→版本，仅审计用）
  kind?: 'bundled'                 // 统一为工程化（bundled）
  installedAt: number
