 ...
 * 动态插件清单（K5 自修改）。
 *
 * 对应 DSH extensions 的 cordis_inspect / cordis_define / cordis_stop / cordis_undefine：
 * 只读报告、记录 package、撤回、遗忘。cordis_run 的 vm 沙箱评估在 runtime 层集成审批后执行。
 */
export class PluginInventory {
  ...
  /** cordis_inspect：只读报告（services 由上层注入，packages 为动态包列表） */
